Both files are actually the same canonical JSON — you didn't attach `format.ts`. But I have everything I need from the canonical to rewrite it correctly. The canonical structure is clear now. Replace your entire `lib/format.ts` with this:
...
The problems in your old `format.ts` were:

1. **Parties** — looking for `canonical.parties.clientName` but your backend uses `canonical.parties.client.name` (nested object)
2. **Scope** — missing `deliverables`, `milestones`, `description` entirely
3. **Commercials** — missing `pricingModel`, `taxes`, number formatting
4. **Security** — using wrong key names (`complianceStandard` vs `complianceStandards`, `liabilityLimit` vs `liabilityCap`)
5. **Conflicts** — the `alternatives[].value` can be a string or an array, wasn't handled
6. **Missing fields** — wasn't grouping `projectGovernance` correctly (case mismatch)

This rewrite maps exactly to your canonical shape.
